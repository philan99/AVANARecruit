...
- `artifacts/insights/` — **AVANA Insights**, AI-native data intelligence platform (react-vite kind, previewPath `/insights/`, slug `insights`). Phase 1 foundation only. Brand: navy `#1A2035`, green `#4CAF50`, Inter font; logo `attached_assets/AVANA_Insights_Logo_1777405980691.png`. Reuses existing `/api/auth/login` (custom email/password + bcrypt + `user_sessions` table) with the SHARED `avanatalent_*` localStorage prefix so a single sign-in works across `/web/` and `/insights/`. Pages: `landing.tsx`, `home.tsx` (KPI tiles + quick actions), `settings.tsx` (Profile/Workspace/Members tabs). `data-sources/explorer/query/decisions/dashboards` are placeholder routes for Phase 2-6. Frontend → API calls all use the absolute prefix `/api` (NOT `${BASE_URL}api`) because the proxy routes most-specific-prefix-first and `/insights/api/...` would be served by the insights vite dev server, not the api-server. New API routes in `artifacts/api-server/src/routes/insights.ts`: `POST /api/insights/bootstrap` (token in body; idempotent + race-safe via UNIQUE(`owner_email`) and a transaction with `ON CONFLICT DO NOTHING`; auto-provisions a workspace named `<name>'s Workspace` and inserts the owner as the first member row, then returns the workspace) and `GET /api/insights/workspace/:id/members` (Authorization: Bearer <token> ONLY; query-string token rejected). New schema in `lib/db/src/schema/insights.ts`: `insights_workspaces` (UNIQUE on `owner_email`), `insights_workspace_members` (UNIQUE on `(workspace_id, member_email)`), `insights_data_sources`, `insights_sync_logs`, `insights_queries`, `insights_decisions`, `insights_dashboards`, `insights_widgets`, `insights_alerts`. Phases 2-6 (data sources, query, decisions, dashboards) will use the Replit Anthropic AI integration via `.local/skills/ai-integrations-anthropic`.
