...
- **Experience matching** (20% weight): Driven entirely by **role-relevant years** — work-history entries scored for relevance (AI-cached, with heuristic fallback) against the job, then summed as `Σ (recency-weighted entry duration × entry relevance)` and run through the years-vs-requirement curve. Unrelated tenure does **not** contribute: a 20-year career in another field scores the same as no career at all if none of it is relevant. The only exception is when a candidate has no work-history entries on file at all — in that case we fall back to declared `experienceYears` because there is no other signal. The assessment text explicitly calls out the "no role-relevant experience" case.
